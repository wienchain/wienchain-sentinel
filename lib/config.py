...
sentinel_cfg = WienchainConfig.tokenize(sentinel_config_file)
sentinel_version = "1.4.0"


def get_wienchain_conf():
    if sys.platform == 'win32':
        wienchain_conf = os.path.join(os.getenv('APPDATA'), "Wienchain/wienchain.conf")
    else:
        home = os.environ.get('HOME')

        wienchain_conf = os.path.join(home, ".wienchain/wienchain.conf")
        if sys.platform == 'darwin':
            wienchain_conf = os.path.join(home, "Library/Application Support/Wienchain/wienchain.conf")

    wienchain_conf = sentinel_cfg.get('wienchain_conf', wienchain_conf)

    return wienchain_conf
...
wienchain_conf = get_wienchain_conf()
network = get_network()
