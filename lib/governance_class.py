...
class GovernanceClass(object):
    ...
    def vote(self, wienchaind, signal, outcome):
        return self.go.vote(wienchaind, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, wienchaind):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(wienchaind, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(wienchaind, models.VoteSignals.valid, models.VoteOutcomes.no)
