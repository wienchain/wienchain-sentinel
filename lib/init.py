...
def has_wienchain_conf():
    import config
    import io

    valid_wienchain_conf = False

    # ensure wienchain_conf exists & readable
    #
    # if not, print a message stating that Wienchain Core must be installed and
    # configured, including JSONRPC access in wienchain.conf
    try:
        f = io.open(config.wienchain_conf)
        valid_wienchain_conf = True
    except IOError as e:
        print(e)

    return valid_wienchain_conf
...
def main():
    ...
    if not has_wienchain_conf():
        print("WienchainCore must be installed and configured, including JSONRPC access in wienchain.conf")
        sys.exit(1)
