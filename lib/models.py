...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, wienchaind):
        golist = wienchaind.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_wienchaind(wienchaind, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_wienchaind(self, wienchaind, rec):
        import decimal
        import wienchainlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from wienchaind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from wienchaind, with every run
        ...
        # get/create, then sync payment amounts, etc. from wienchaind - Wienchaind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(wienchaind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from wienchaind! %s" % e)
            govobj.vote_delete(wienchaind)
            return (govobj, None)
    ...
    def vote_delete(self, wienchaind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(wienchaind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, wienchaind, signal, outcome):
        import wienchainlib
        ...
        output = wienchaind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = wienchainlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(wienchaind, signal)

    def sync_network_vote(self, wienchaind, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = wienchaind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 wienchain addr, non-multisig
            if not wienchainlib.is_valid_wienchain_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Wienchain address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Wienchain Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (wienchainlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not wienchainlib.is_valid_wienchain_address(addr, config.network):
    ...
    def hash(self):
        import wienchainlib
        return wienchainlib.hashit(self.serialise())
