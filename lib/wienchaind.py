...
class WienchainDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some wienchaind methods
    ...
    @classmethod
    def from_wienchain_conf(self, wienchain_dot_conf):
        from wienchain_config import WienchainConfig
        config_text = WienchainConfig.slurp_config_file(wienchain_dot_conf)
        creds = WienchainConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = wienchainlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = wienchainlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = wienchainlib.blocks_to_seconds(diff)
