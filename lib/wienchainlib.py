...
def is_valid_wienchain_address(address, network='mainnet'):
    ...
    wienchain_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Wienchain address
        return False

    if (address_version != wienchain_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('wienchain.conf')
